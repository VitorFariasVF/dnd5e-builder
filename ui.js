...
function renderClasse(){const options=Object.entries(CLASSES).map(([k,i])=>`<option value="${k}" ${state.personagem.classe===k?"selected":""}>${i.nome}</option>`).join(""); const nivelAtual=state.personagem.nivel||1; const nivelOptions=Array.from({length:20},(_,i)=>i+1).map(n=>`<option value="${n}" ${nivelAtual===n?"selected":""}>Nível ${n}</option>`).join(""); renderPassoBase("Classe",`<label for="classe">Selecione uma classe</label><select id="classe" onchange="mostrarPreviewClasse()"><option value="">Selecione</option>${options}</select><label for="nivel">Nível inicial / atual</label><select id="nivel" onchange="mostrarPreviewClasse()">${nivelOptions}</select><div id="preview"></div><div class="actions"><button class="secondary" onclick="voltarPara(3)">Voltar</button><button onclick="confirmarClasse()">Avançar</button></div>`); mostrarPreviewClasse();}
function mostrarPreviewClasse(){const key=document.getElementById("classe").value; const nivel=Number(document.getElementById("nivel")?.value || state.personagem.nivel || 1); const p=document.getElementById("preview"); if(!key||!CLASSES[key]) return p.innerHTML=""; const c=CLASSES[key]; const features=getClassFeaturesForLevel(key,nivel); const attacksPerAction=getAttacksPerAction(key,nivel); const slots=SPELLCASTERS[key] ? (key==="bruxo" ? `${SPELL_SLOTS.warlock[nivel][0]||0} espaço(s) de pacto de ${SPELL_SLOTS.warlockCircle[nivel]}º círculo` : (SPELLCASTERS[key].meioConjurador?SPELL_SLOTS.half[nivel]:SPELL_SLOTS.full[nivel]).map((v,i)=>v?`${i+1}º:${v}`:"").filter(Boolean).join(" • ") || "Sem espaços") : "Não"; const unlock=getSubclassUnlockLevel(key); const subclasses=getSubclassOptions(key); const selected=document.getElementById("subclasse")?.value || state.personagem.subclasse || ""; const shouldShowSub=nivel>=unlock && subclasses.length; const subSelect=shouldShowSub ? `<label for="subclasse">Subclasse</label><select id="subclasse" onchange="mostrarPreviewClasse()"><option value="">Selecione</option>${subclasses.map(s=>`<option value="${s.key}" ${selected===s.key?"selected":""}>${s.nome}</option>`).join("")}</select>` : `<p class="muted">Subclasse desbloqueia no nível ${unlock}.</p>`; const sub=getSubclassByKey(key, selected); const recursos=getClassResources(key, nivel, state.personagem); const profsCombinadas=(typeof getAllCharacterProficiencies==='function') ? getAllCharacterProficiencies({...state.personagem, classe:key, subclasse:selected, nivel}) : c.proficiencias; const notasSub=(typeof getSubclassProficiencyNotes==='function') ? getSubclassProficiencyNotes({...state.personagem, classe:key, subclasse:selected, nivel}) : []; p.innerHTML=`<div class="info-box"><p><strong>Dado de vida:</strong> d${c.dadoVida}</p><p><strong>Resistências:</strong> ${c.saves.join(", ")}</p><p><strong>Proficiências:</strong> ${profsCombinadas.join(", ")}</p><p><strong>Perícias da classe:</strong> escolha ${c.escolhaPericias.quantidade}</p><p><strong>Conjuração:</strong> ${SPELLCASTERS[key] ? "Sim" : "Não"}</p><p><strong>Espaços no nível ${nivel}:</strong> ${slots}</p><p><strong>Ataques por ação:</strong> ${attacksPerAction}</p><p><strong>Equipamento inteligente:</strong> ${(EQUIPMENT_LOADOUTS[key]||[]).length} grupo(s) de escolha</p>${subSelect}<p><strong>Habilidades de classe no nível ${nivel}:</strong></p>${(features.map(f=>`<div><strong>${f.nome}</strong> <small class="muted">(nível ${f.level})</small></div>`).join("") || `<div class="muted">Nenhuma listada.</div>`)}${sub?`<div class="card-nested"><p><strong>${sub.nome}</strong></p><p class="muted">${sub.resumo}</p>${notasSub.length?`<p><strong>Proficiências e bônus da subclasse:</strong><br><small>${notasSub.join('<br>')}</small></p>`:''}${(getSubclassFeaturesForLevel(key, selected, nivel).map(f=>`<div><strong>${f.nome}</strong> <small class="muted">(nível ${f.level})</small></div>`).join("") || `<div class="muted">Sem traços ativos.</div>`)}</div>`:""}${recursos.length?`<div class="card-nested"><p><strong>Recursos rastreáveis</strong></p>${recursos.map(r=>`<div>${r.nome}: ${r.max}/${r.max} • ${r.recarga}</div>`).join("")}</div>`:""}</div>`;}
function confirmarClasse(){const key=document.getElementById("classe").value; state.personagem.classe=key; state.personagem.nivel=Math.max(1, Math.min(20, Number(document.getElementById("nivel")?.value || 1))); state.personagem.subclasse=document.getElementById("subclasse")?.value || null; const erro=validarClasse(); if(erro&&state.modo==="A") return alert(erro); if(erro&&state.modo==="B") state.personagem.customizado=true; if(key&&CLASSES[key]){const c=CLASSES[key]; state.personagem.dadoVida=c.dadoVida; state.personagem.savesClasse=[...c.saves]; state.personagem.proficienciasClasse=[...c.proficiencias]; state.personagem.ferramentasAtivas=(typeof getAllCharacterTools==='function') ? getAllCharacterTools(state.personagem) : [...(state.personagem.ferramentas||[])]; state.personagem.equipamentoClasse=[...c.equipamento]; state.personagem.periciasClasseSelecionadas=[]; state.personagem.equipamentoInicialEscolhas={}; state.personagem.equipamentoInicialValores={}; configurarMagiaDaClasse(); recalcularTudo();} avancarPara(5);} 
...
function atualizarPericiasFinais(){ state.personagem.periciasFinais=(typeof getAllCharacterSkills==='function') ? getAllCharacterSkills(state.personagem) : [...new Set([...state.personagem.periciasOrigem,...state.personagem.periciasClasseSelecionadas])]; state.personagem.ferramentasAtivas=(typeof getAllCharacterTools==='function') ? getAllCharacterTools(state.personagem) : [...new Set(state.personagem.ferramentas||[])]; }
...
function recalcularSaves(){const profs=(typeof getAllCharacterSaveProficiencies==='function') ? getAllCharacterSaveProficiencies(state.personagem) : (state.personagem.savesClasse||[]); const saves={}; for(const [label,key] of Object.entries(SAVE_TO_KEY)){let valor=calcularMod(state.personagem.atributos[key]||10); if(profs.includes(label)) valor+=state.personagem.bonusProficiencia; saves[label]=valor;} state.personagem.savesCalculados=saves;}
...
function mostrarPreviewClasse(){const key=document.getElementById("classe").value; const nivel=Number(document.getElementById("nivel")?.value || state.personagem.nivel || 1); const p=document.getElementById("preview"); if(!key||!CLASSES[key]) return p.innerHTML=""; const c=CLASSES[key]; const selected=document.getElementById("subclasse")?.value || state.personagem.subclasse || ""; state.personagem.classe=key; state.personagem.nivel=nivel; state.personagem.subclasse=selected || null; state.personagem.opcoesClasse=sanitizeClassOptionState(state.personagem); const features=getClassFeaturesForLevel(key,nivel); const attacksPerAction=getAttacksPerAction(key,nivel); const slots=SPELLCASTERS[key] ? (key==="bruxo" ? `${SPELL_SLOTS.warlock[nivel][0]||0} espaço(s) de pacto de ${SPELL_SLOTS.warlockCircle[nivel]}º círculo` : (SPELLCASTERS[key].meioConjurador?SPELL_SLOTS.half[nivel]:SPELL_SLOTS.full[nivel]).map((v,i)=>v?`${i+1}º:${v}`:"").filter(Boolean).join(" • ") || "Sem espaços") : "Não"; const unlock=getSubclassUnlockLevel(key); const subclasses=getSubclassOptions(key); const shouldShowSub=nivel>=unlock && subclasses.length; const subSelect=shouldShowSub ? `<label for="subclasse">Subclasse</label><select id="subclasse" onchange="mostrarPreviewClasse()"><option value="">Selecione</option>${subclasses.map(s=>`<option value="${s.key}" ${selected===s.key?"selected":""}>${s.nome}</option>`).join("")}</select>` : `<p class="muted">Subclasse desbloqueia no nível ${unlock}.</p>`; const sub=getSubclassByKey(key, selected); const recursos=getClassResources(key, nivel, state.personagem); const opcoesHtml = renderClassOptionsEditor(key, nivel, selected); p.innerHTML=`<div class="info-box"><p><strong>Dado de vida:</strong> d${c.dadoVida}</p><p><strong>Resistências:</strong> ${c.saves.join(", ")}</p><p><strong>Proficiências:</strong> ${profsCombinadas.join(", ")}</p><p><strong>Perícias da classe:</strong> escolha ${c.escolhaPericias.quantidade}</p><p><strong>Conjuração:</strong> ${SPELLCASTERS[key] ? "Sim" : "Não"}</p><p><strong>Espaços no nível ${nivel}:</strong> ${slots}</p><p><strong>Ataques por ação:</strong> ${attacksPerAction}</p><p><strong>Equipamento inteligente:</strong> ${(EQUIPMENT_LOADOUTS[key]||[]).length} grupo(s) de escolha</p>${subSelect}<p><strong>Habilidades de classe no nível ${nivel}:</strong></p>${(features.map(f=>`<div><strong>${f.nome}</strong> <small class="muted">(nível ${f.level})</small></div>`).join("") || `<div class="muted">Nenhuma listada.</div>`)}${sub?`<div class="card-nested"><p><strong>${sub.nome}</strong></p><p class="muted">${sub.resumo}</p>${notasSub.length?`<p><strong>Proficiências e bônus da subclasse:</strong><br><small>${notasSub.join('<br>')}</small></p>`:''}${(getSubclassFeaturesForLevel(key, selected, nivel).map(f=>`<div><strong>${f.nome}</strong> <small class="muted">(nível ${f.level})</small></div>`).join("") || `<div class="muted">Sem traços ativos.</div>`)}</div>`:""}${recursos.length?`<div class="card-nested"><p><strong>Recursos rastreáveis</strong></p>${recursos.map(r=>`<div>${r.nome}: ${r.max}/${r.max} • ${r.recarga}</div>`).join("")}</div>`:""}<div class="card-nested"><p><strong>Escolhas e recursos configuráveis</strong></p>${opcoesHtml}</div></div>`;}
...
function renderResumo(){ recalcularTudo(); state.personagem.validacaoResumo=validarPersonagemCompleto(); state.personagem.valido=state.personagem.validacaoResumo.status==="oficial"; const p=state.personagem; const status=p.validacaoResumo.status==="oficial"?`<div class="alert success">✔ personagem válido</div>`:p.validacaoResumo.status==="customizado"?`<div class="alert warning">⚠ personagem customizado</div>`:`<div class="alert error">⚠ personagem com pendências</div>`; const atributos=ATTRIBUTE_LIST.map(attr=>`<li>${ATTRIBUTE_LABELS[attr]}: ${p.atributos[attr]} (mod ${valorFormatadoBonus(calcularMod(p.atributos[attr]))})</li>`).join(""); const ataques=p.combate.ataques.map(a=>`<div class="attack-card"><h4>${a.nome}</h4><p><strong>Bônus:</strong> ${valorFormatadoBonus(a.bonusAtaque)}</p><p><strong>Dano:</strong> ${a.dano}</p>${a.danoVersatil?`<p><strong>Dano versátil:</strong> ${a.danoVersatil}</p>`:""}<p><strong>Crítico:</strong> ${a.critico||"-"}</p><p><strong>Valor:</strong> ${formatarMoedaGp(a.valorGp||0)}</p><p><strong>Atributo:</strong> ${a.atributo||"-"}</p><p><strong>Proficiência:</strong> ${a.proficiente?"Sim":"Não"}</p><p><strong>Alcance:</strong> ${a.alcance}</p><p><strong>Propriedades:</strong> ${a.propriedades.join(", ")||"Nenhuma"}</p></div>`).join(""); const inventario=p.inventario.itens.map(item=>`<li>${item.nome} x${item.quantidade} — peso ${item.pesoTotal}</li>`).join("")||"<li>Nenhum item.</li>"; const magia = !p.magia.ehConjurador ? "<p>Classe sem conjuração.</p>" : `<p><strong>Habilidade:</strong> ${ATTRIBUTE_LABELS[p.magia.habilidade]}</p><p><strong>CD:</strong> ${p.magia.cdMagia}</p><p><strong>Ataque mágico:</strong> ${valorFormatadoBonus(p.magia.ataqueMagia)}</p><p><strong>Espaços:</strong> ${p.magia.espacos.map((v,i)=>v?`${i+1}º:${p.magia.espacosAtuais[i]||0}/${v}`:"").filter(Boolean).join(" • ") || "Nenhum"}</p><p><strong>Truques:</strong> ${(p.magia.listaTruques||[]).join(", ") || "Nenhum"}</p><p><strong>Magias:</strong> ${(p.magia.listaMagias||[]).join(", ") || "Nenhuma"}</p>`; const pendencias = `<ul class="clean">${(p.validacaoResumo.erros||[]).map(e=>`<li>${e}</li>`).join("") || '<li>Nenhuma pendência crítica.</li>'}</ul>`; const avisos = `<ul class="clean">${(p.validacaoResumo.avisos||[]).map(e=>`<li>${e}</li>`).join("") || '<li>Nenhum aviso.</li>'}</ul>`; const subclasseNome=nomeCatalogo(Object.fromEntries(getSubclassOptions(p.classe).map(s=>[s.key,{nome:s.nome}])), p.subclasse); renderPassoBase("Resumo da Ficha",`${status}<div class="grid grid-2"><div><h3>Identidade</h3><p><strong>Nome:</strong> ${p.nome||'-'}</p><p><strong>Origem:</strong> ${nomeCatalogo(BACKGROUNDS,p.origem)}</p><p><strong>Raça:</strong> ${nomeCatalogo(RACES,p.raca)}</p><p><strong>Classe:</strong> ${nomeCatalogo(CLASSES,p.classe)} ${p.nivel}</p><p><strong>Subclasse:</strong> ${subclasseNome}</p><p><strong>Progressão:</strong> próximo nível ${p.progressao.proximoNivel} • bônus de proficiência ${valorFormatadoBonus(p.bonusProficiencia)} • ataques por ação ${p.progressao.ataquesPorAcao||1}</p><h4>Melhorias pendentes</h4><ul class="clean">${(p.progressao.melhoriasPendentes||[]).map(item=>`<li>${item}</li>`).join("") || "<li>Nenhuma no nível atual.</li>"}</ul><h3>Habilidades de Classe</h3>${renderClassFeaturesHtml()}<h3>Subclasse</h3>${renderSubclassFeaturesHtml()}${(typeof getSubclassProficiencyNotes==='function' && getSubclassProficiencyNotes(p).length)?`<div class="card-nested"><strong>Proficiências da subclasse</strong><br><small>${getSubclassProficiencyNotes(p).join('<br>')}</small></div>`:''}<h3>Proficiências ativas</h3><p><strong>Armas/Armaduras:</strong> ${((typeof getAllCharacterProficiencies==='function')?getAllCharacterProficiencies(p):(p.proficienciasClasse||[])).join(', ') || '-'}</p><p><strong>Ferramentas:</strong> ${((typeof getAllCharacterTools==='function')?getAllCharacterTools(p):(p.ferramentas||[])).join(', ') || '-'}</p><h3>Recursos</h3>${renderRecursosClasseHtml()}<h3>Escolhas de classe</h3>${renderClassOptionsResumoHtml()}<h3>Atributos</h3><ul class="clean">${atributos}</ul><h3>Combate</h3><p><strong>PV:</strong> ${p.pvMax}</p><p><strong>CA:</strong> ${p.combate.classeArmadura}</p><p><strong>Iniciativa:</strong> ${valorFormatadoBonus(p.combate.iniciativa)}</p><p><strong>Deslocamento:</strong> ${p.deslocamento} pés</p><div class="grid grid-2">${ataques}</div><h3>Validação</h3>${pendencias}<h4>Avisos</h4>${avisos}</div><div><h3>Inventário</h3><ul class="clean">${inventario}</ul><p><strong>Peso:</strong> ${p.inventario.pesoTotal}/${p.inventario.capacidadeCarga}</p><p><strong>Moedas:</strong> PP ${p.inventario.moedas.pp}, GP ${p.inventario.moedas.gp}, EP ${p.inventario.moedas.ep}, SP ${p.inventario.moedas.sp}, CP ${p.inventario.moedas.cp}</p><h3>Magia</h3>${magia}<h3>Personalidade</h3><p><strong>Traços:</strong> ${(p.personalidade.tracos||[]).join(' | ')||'-'}</p><p><strong>Ideais:</strong> ${(p.personalidade.ideais||[]).join(' | ')||'-'}</p><p><strong>Vínculos:</strong> ${(p.personalidade.vinculos||[]).join(' | ')||'-'}</p><p><strong>Defeitos:</strong> ${(p.personalidade.defeitos||[]).join(' | ')||'-'}</p><p><strong>Descrição:</strong> ${p.personalidade.descricaoGeral||'-'}</p><p><strong>Aparência:</strong> ${p.personalidade.aparencia||'-'}</p><p><strong>História:</strong> ${p.personalidade.historia||'-'}</p></div></div><div class="actions"><button class="secondary" onclick="voltarPara(10)">Voltar</button><button onclick="exportarFoundryActorJSON()">Exportar JSON Foundry</button><button class="secondary" onclick="exportarBackupJSON()">Exportar JSON Backup</button><button onclick="exportarPDF()">Exportar PDF</button><button class="secondary" onclick="reiniciar()">Criar novo personagem</button></div>`); salvarEstado(); }
