...
function buildClassPreviewHtml(key, nivel, selectedOverride){
  try {
    ...
    const slotPreviewState = {...state.personagem, classe:key, nivel, subclasse:selected || null};
    const slots = (typeof getRuleEngineSpellSlotSummary === "function")
      ? getRuleEngineSpellSlotSummary(slotPreviewState)
      : (SPELLCASTERS[key]
        ? (key === "bruxo"
...
function inicializarEscolhasEquipamentoInicial(){
  ...
  state.personagem.equipamentoInicialEscolhas = state.personagem.equipamentoInicialEscolhas || {};
  state.personagem.equipamentoInicialValores = state.personagem.equipamentoInicialValores || {};
  loadout.forEach(group => {
    if(group.choices.length === 1 && (state.personagem.equipamentoInicialEscolhas[group.id] === '' || state.personagem.equipamentoInicialEscolhas[group.id] == null)){
      state.personagem.equipamentoInicialEscolhas[group.id] = '0';
    }
    if(state.personagem.equipamentoInicialValores[group.id] == null) state.personagem.equipamentoInicialValores[group.id] = false;
  });
}

function renderCombate(){
  recalcularTudo();
  inicializarEscolhasEquipamentoInicial();
...
function renderEquipamentoInicial(){ const loadout=EQUIPMENT_LOADOUTS[state.personagem.classe]||[]; if(!loadout.length) return ""; return `<h3>Equipamento inicial inteligente</h3><p class="muted">Escolhas obrigatórias da classe. Opções com item único já vêm pré-selecionadas para não travar o avanço.</p><div class="grid">${loadout.map(group=>{ const selectedIndex=String(state.personagem.equipamentoInicialEscolhas[group.id]??""); const selectedChoice=group.choices[Number(selectedIndex)]; const valor=selectedChoice?getChoiceValueGp(selectedChoice):0; const pending = !selectedChoice; return `<div class="card-nested ${pending?'card-pending':''}"><label>${group.label}<select id="equip_${group.id}" onchange="atualizarPreviewEquipamentoInicial()"><option value="">Selecione</option>${group.choices.map((choice,idx)=>`<option value="${idx}" ${selectedIndex===String(idx)?"selected":""}>${choice.label} • ${formatarMoedaGp(getChoiceValueGp(choice))}</option>`).join("")}</select></label><label class="checkbox-row"><input type="checkbox" id="equip_valor_${group.id}" ${(state.personagem.equipamentoInicialValores?.[group.id])?"checked":""} onchange="atualizarPreviewEquipamentoInicial()" /> Receber valor em vez dos itens ${selectedChoice?`(${formatarMoedaGp(valor)})`:''}</label><div id="equip_preview_${group.id}" class="muted">${selectedChoice?renderChoiceItemsPreview(selectedChoice):"Selecione uma opção para ver os detalhes."}</div>${pending?'<p class="muted">Pendência: escolha uma opção para este grupo.</p>':''}</div>`; }).join("")}</div>`; }
function confirmarCombate(){ state.personagem.combate.armadura=document.getElementById("armadura").value; state.personagem.combate.armaPrincipal=document.getElementById("armaPrincipal").value; state.personagem.combate.armaSecundaria=document.getElementById("armaSecundaria").value; state.personagem.combate.escudo=document.getElementById("escudo").checked; inicializarEscolhasEquipamentoInicial(); capturarEscolhasEquipamentoInicial(); aplicarEquipamentoInicial(); recalcularTudo(); const erro=validarCombate()||validarEquipamentoInicial(); if(erro&&state.modo==="A") return alert(erro); if(erro&&state.modo==="B") state.personagem.customizado=true; avancarPara(8);} 
...
function recalcularPv(){
  if(typeof resolveAndApplyRuleEngineHitPoints === "function") {
    resolveAndApplyRuleEngineHitPoints(state.personagem);
    if(Number.isFinite(Number(state.personagem.pvMax)) && Number(state.personagem.pvMax) > 0) return;
  }
  const dado=(CLASSES[state.personagem.classe]?.dadoVida || state.personagem.dadoVida || 0);
  const nivel=Math.max(1,state.personagem.nivel||1);
  const modCon=calcularMod(state.personagem.atributos.constituicao||10);
  const mediaPorNivel=Math.floor(dado/2)+1;
  const primeiroNivel=Math.max(1,dado+modCon);
  const ganhoPorNivel=Math.max(1,mediaPorNivel+modCon);
  state.personagem.pvMax=dado?Math.max(1,primeiroNivel+((nivel-1)*ganhoPorNivel)):null;
}
...
function renderClassOptionsEditor(classe, nivel, subclasse, personagemRef){
  ...
  const atual = (personagemRef?.opcoesClasse) || state.personagem.opcoesClasse || {};
...
function mostrarPreviewClasse(){
  ...
  const previousOptions = typeof coletarOpcoesClasseDaTela === 'function' ? coletarOpcoesClasseDaTela() : (state.personagem.opcoesClasse || {});
  ...
  state.personagem.opcoesClasse = {...state.personagem.opcoesClasse, ...previousOptions};
...
function prepararFichaImportada(){
  ...
  if (p.classe && CLASSES[p.classe]) {
    ...
    p.savesClasseBase = [...c.saves];
    p.savesClasse = [...c.saves];
    p.proficienciasClasseBase = [...c.proficiencias];
